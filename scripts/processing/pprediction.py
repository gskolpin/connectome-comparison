...
#SHOULD put in a random state and care about that oh god
...
# %%
network_connectomes = get_connectomes(con_model='lassoBIC_blocks', network=True, network_labels=labels_7)
network_connectomes.head()

# %%
network_17_connectomes = get_connectomes(con_model='lassoBIC_blocks', network=True, network_labels=labels_17)
network_17_connectomes.head()

# %%
corr_net_connectomes = get_connectomes(con_model='correlation_random', network=True, network_labels=labels_7)
corr_net_connectomes.head()
...
# should do this through nilearn
...
ids = lasso_connectomes.index
c1 = axs[0].imshow(np.reshape(network_connectomes.loc[ids[0]], (7, 7)), vmin=-1, vmax=1, cmap='bwr')
c2 = axs[1].imshow(np.reshape(corr_net_connectomes.loc[ids[0]], (7, 7)), vmin=-1, vmax=1, cmap='bwr')
c3 = axs[2].imshow(np.reshape(network_17_connectomes.loc[ids[0]], (17, 17)), vmin=-1, vmax=1, cmap='bwr')
cbar = f.colorbar(c1, ax=axs, shrink=0.5)
...
# %%
#scikit score function? Yes each has a seperate one, soooo 
'''
...
# %%
#try passing in model object and see
def fit_model(X, y, folds, model_obj):
    '''
    TO DO
    '''
    kf = KFold(folds)
...
# maybe use the indeces to make sure they are ordered the same?
...
network_7_data = pd.concat([network_connectomes, rand100_phenotype], join='inner', axis=1)
network_17_data = pd.concat([network_17_connectomes, rand100_phenotype], join='inner', axis=1)
...
lasso_results = fit_model(lasso_data[[f'edge_{num}' for num in range(10000)]], lasso_data['age'], 5, model)
corr_results = fit_model(corr_data[[f'edge_{num}' for num in range(10000)]], corr_data['age'], 5, model)
network_7_results = fit_model(network_7_data[network_connectomes.columns], network_7_data['age'], 5, model)
network_17_results = fit_model(network_17_data[network_17_connectomes.columns], network_17_data['age'], 5, model)

# %%
#interested in the alphas lassoCV chooses, seems like the sparce connectomes are just overfitting so maybe can get good prediction from that
#print(model.alphas_)

# %%
#seems like they only went to a bit over 1, double checking the alphas it uses (seems like the default works fine)
alphas = np.logspace(-4, 10, 100)
reg_model = LassoCV(random_state=random_state, alphas=alphas)
morereg_lasso_results = fit_model(lasso_data[[f'edge_{num}' for num in range(10000)]], lasso_data['age'], 5, reg_model)
print(reg_model.alpha_)
#print(reg_model.alphas_)
print(morereg_lasso_results['fold 1 stats'])

# %%
# make some new ones for comparison
print('lasso:', lasso_results['fold 1 stats'], 'full r2:', lasso_results['full_prediction_r2']) #looks just like a lot of overfitting
print('correlation:', corr_results['fold 1 stats'], 'full r2:', corr_results['full_prediction_r2']) #um huh, really low train, but also overfitting, maybe just not enough subjects???
print('lasso 7 networks:', network_7_results['fold 1 stats'], 'full r2:', network_7_results['full_prediction_r2'])
print('lasso 17 networks:', network_17_results['fold 1 stats'], 'full r2:', network_17_results['full_prediction_r2'])
...
f, (ax1, ax2) = plt.subplots(ncols=2, sharex=True, sharey=True, figsize=(10, 5))
ax1.scatter(network_7_results['k_fold_age'], network_7_results['predictions'], alpha=0.5)
n7_b, n7_a = np.polyfit(network_7_results['k_fold_age'], network_7_results['predictions'], 1)
ax1.plot(network_7_results['k_fold_age'], n7_a + n7_b * network_7_data['age'], color='red')

ax2.scatter(network_17_results['k_fold_age'], network_17_results['predictions'], alpha=0.5)
n17_b, n17_a = np.polyfit(network_17_results['k_fold_age'], network_17_results['predictions'], 1)
ax2.plot(network_17_results['k_fold_age'], n17_a + n17_b * network_17_data['age'], color='red')
...
print(98 - np.count_nonzero(lasso_connectomes['edge_4407']))
